...
# get active power
start_t = time.time()
start = '2019-07-08 08:50:00.000'
end = '2019-07-08 08:55:00.000'

df_act = Select('HisItemVibra', start, end)
...
#%%
# split active power date
start_date, start_time, end_time, start_temp, end_temp = [], [], [], [], []
for i in range(len(act_date)):
   start_split= str(act_date[i][0]).split()
   end_split = str(act_date[i][len(act_date[i])-1]).split()
   ...
   start_time = make_string(start_temp)
   end_time = make_string(end_temp)
